d3.sankey = function() {
  ...
  function computeNodeBreadths() {
    ...
    nodes
      ...
      // .forEach(function(node) { node.x = max; })  // KAK don't shift top preds to right
  ...
  function moveSourcesRight() {
    nodes.forEach(function(node) {
      if (!node.targetLinks.length) {
		  // node.x = d3.min(node.sourceLinks, function(d) { return d.target.x; }) - 1; // KAK Don't shift producers/detrital left
      }
    });
  }
